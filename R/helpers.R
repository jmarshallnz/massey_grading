...
grade_factor <- function(grades) {
  factor(grades, levels=c("A+", "A", "A-", "B+", "B", "B-", "C+", "C", "C-", "D", "E", "MA", "F", "DC", "AG", "WD"))
}

#' Computes the grade category (A, B, C, F, DC) for a vector of grades
#' @param grades the grades as a character vector
...
grade_category <- function(grades) {
  ...
  factor(grade_cat, levels = c("A", "B", "C", "F", "DC"))
}
